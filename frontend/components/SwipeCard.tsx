    ...
    >
      ...
      >
          ...
          <div className="flex items-start justify-between gap-3">
            ...
            {project.tier && (
              <div className="flex flex-shrink-0 flex-col items-center gap-1.5">
                {project.compatibility_score != null && (
                  <ScoreRing score={project.compatibility_score} tierColor={project.tier.color} size={64} stroke={6} />
                )}
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          {/* Why this score? — inline disclosure */}
          {project.compatibility_score != null &&
            project.score_breakdown &&
            project.score_breakdown.length > 0 && (
              <div className="rounded-xl border border-slate-100 bg-slate-50">
                ...
                >
                  <span>Why {project.compatibility_score}% match?</span>
                  <span className="text-slate-400">{showScore ? "▲" : "▼"}</span>
                </button>
                {showScore && (
                  <div className="px-3 pb-3 pt-1">
                    <ScoreBreakdown breakdown={project.score_breakdown} />
                  </div>
                )}
              </div>
            )}
